  ...
  return(<div style={bS.page}>
    ...
    <div style={{maxWidth:900,margin:"0 auto",padding:16}}>
      {tab==="home"&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14}}>
        ...
        <div>
          <div style={bS.card}><div style={bS.h2}>Quick Actions</div><button style={{...bS.btn(),width:"100%",marginBottom:10,textAlign:"left"}} onClick={()=>{setShowS(!showS);setShowR(false);}}>Suggest a Meal</button><button style={{...bS.btnGhost,width:"100%",textAlign:"left"}} onClick={()=>{setShowR(!showR);setShowS(false);}}>Request Shopping Item</button></div>
          {showS&&<div style={bS.card}><div style={bS.h2}>Suggest a Meal</div><div style={{marginBottom:8}}><div style={bS.label}>Meal</div><input style={bS.input} placeholder="e.g. Lasagna, Tacos..." value={sugg.meal} onChange={e=>setSugg({...sugg,meal:e.target.value})}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}><div><div style={bS.label}>Day</div><select style={bS.select} value={sugg.dayPreference} onChange={e=>setSugg({...sugg,dayPreference:e.target.value})}>{DAYS.map(d=><option key={d}>{d}</option>)}</select></div><div><div style={bS.label}>Meal</div><select style={bS.select} value={sugg.mealType} onChange={e=>setSugg({...sugg,mealType:e.target.value})}>{MEAL_TYPES.map(m=><option key={m}>{m}</option>)}</select></div></div><div style={{marginBottom:10}}><div style={bS.label}>Notes</div><input style={bS.input} placeholder="Why you want it..." value={sugg.notes} onChange={e=>setSugg({...sugg,notes:e.target.value})}/></div><div style={{display:"flex",gap:8}}><button style={bS.btn()} onClick={sendSugg}>Submit</button><button style={bS.btnGhost} onClick={()=>setShowS(false)}>Cancel</button></div></div>}
          {showR&&<div style={bS.card}><div style={bS.h2}>Request Shopping Item</div><div style={{marginBottom:8}}><div style={bS.label}>Item</div><input style={bS.input} placeholder="What do you need?" value={item.name} onChange={e=>setItem({...item,name:e.target.value})}/></div><div style={{marginBottom:8}}><div style={bS.label}>How much?</div><input style={bS.input} placeholder="e.g. 2 boxes..." value={item.qty} onChange={e=>setItem({...item,qty:e.target.value})}/></div><div style={{marginBottom:10}}><div style={bS.label}>Reason</div><input style={bS.input} placeholder="Why?" value={item.notes} onChange={e=>setItem({...item,notes:e.target.value})}/></div><div style={{display:"flex",gap:8}}><button style={bS.btn()} onClick={sendItem}>Submit</button><button style={bS.btnGhost} onClick={()=>setShowR(false)}>Cancel</button></div></div>}
        </div>
      </div>}
...
function ParkerTab({mealPlan,shopRequests,setShopRequests,mealSuggestions,setMealSuggestions,chores,setChores,messages,setMessages,appSettings,onLogout}){
  ...
  const pS={page:{background:"linear-gradient(135deg,#1a0a2e,#0d1a2e)",minHeight:"100vh",fontFamily:"Georgia,serif",color:"#e8e0ff"},card:{background:"rgba(180,79,239,0.1)",border:"1px solid rgba(180,79,239,0.25)",borderRadius:12,padding:16,marginBottom:12},cardSm:{background:"rgba(180,79,239,0.07)",border:"1px solid rgba(180,79,239,0.15)",borderRadius:10,padding:12,marginBottom:8},h2:{fontSize:13,color:"#b44fef",fontWeight:"bold",marginBottom:10},btn:(c="#b44fef")=>({background:c,border:"none",borderRadius:10,padding:"11px 18px",color:"#fff",fontSize:13,cursor:"pointer",fontWeight:"bold",fontFamily:"Georgia,serif",whiteSpace:"nowrap"}),btnGhost:{background:"transparent",border:"1px solid rgba(180,79,239,0.3)",borderRadius:8,padding:"7px 12px",color:"#7a6aaa",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:12},btnDanger:{background:"transparent",border:"1px solid #f4433644",borderRadius:6,padding:"5px 10px",color:"#f44336",fontFamily:"Georgia,serif",fontSize:12,cursor:"pointer"},label:{fontSize:10,color:"#7a6aaa",textTransform:"uppercase",letterSpacing:"0.15em",marginBottom:4,fontFamily:"monospace"},input:{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(180,79,239,0.4)",borderRadius:8,padding:"9px 12px",color:"#e8e0ff",fontFamily:"Georgia,serif",fontSize:13,width:"100%",boxSizing:"border-box",outline:"none"},select:{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(180,79,239,0.4)",borderRadius:8,padding:"9px 12px",color:"#e8e0ff",fontFamily:"Georgia,serif",fontSize:13,width:"100%",boxSizing:"border-box",outline:"none"},row:{display:"flex",justifyContent:"space-between",alignItems:"center"},tag:c=>({background:c+"22",color:c,border:`1px solid ${c}44`,borderRadius:4,padding:"2px 8px",fontSize:11,fontFamily:"monospace"}),alert:c=>({background:c+"18",border:`1px solid ${c}44`,borderRadius:8,padding:"10px 14px",marginBottom:10}),T:{accent:"#b44fef",text:"#e8e0ff",sub:"#7a6aaa",border:"rgba(180,79,239,0.25)",bg:"#1a0a2e"}}
...
function RyderTab({mealPlan,shopRequests,setShopRequests,mealSuggestions,setMealSuggestions,chores,setChores,messages,setMessages,appSettings,onLogout}){
  ...
  const rS={page:{background:"linear-gradient(180deg,#0d1f0d,#1a0d0d)",minHeight:"100vh",fontFamily:"Georgia,serif",color:"#fff9f0"},card:{background:"rgba(255,107,53,0.08)",border:"1px solid rgba(255,107,53,0.2)",borderRadius:14,padding:16,marginBottom:12},cardSm:{background:"rgba(255,107,53,0.05)",border:"1px solid rgba(255,107,53,0.15)",borderRadius:10,padding:12,marginBottom:8},h2:{fontSize:13,color:"#ff6b35",fontWeight:"bold",marginBottom:10},btn:(c="#ff6b35")=>({background:c,border:"none",borderRadius:10,padding:"11px 18px",color:"#fff",fontSize:13,cursor:"pointer",fontWeight:"bold",fontFamily:"Georgia,serif",whiteSpace:"nowrap"}),btnGhost:{background:"transparent",border:"1px solid rgba(255,107,53,0.3)",borderRadius:8,padding:"7px 12px",color:"#887766",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:12},btnDanger:{background:"transparent",border:"1px solid #f4433644",borderRadius:6,padding:"5px 10px",color:"#f44336",fontFamily:"Georgia,serif",fontSize:12,cursor:"pointer"},label:{fontSize:10,color:"#887766",textTransform:"uppercase",letterSpacing:"0.15em",marginBottom:4,fontFamily:"monospace"},input:{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,107,53,0.4)",borderRadius:8,padding:"9px 12px",color:"#fff9f0",fontFamily:"Georgia,serif",fontSize:13,width:"100%",boxSizing:"border-box",outline:"none"},select:{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,107,53,0.4)",borderRadius:8,padding:"9px 12px",color:"#fff9f0",fontFamily:"Georgia,serif",fontSize:13,width:"100%",boxSizing:"border-box",outline:"none"},row:{display:"flex",justifyContent:"space-between",alignItems:"center"},tag:c=>({background:c+"22",color:c,border:`1px solid ${c}44`,borderRadius:4,padding:"2px 8px",fontSize:11,fontFamily:"monospace"}),alert:c=>({background:c+"18",border:`1px solid ${c}44`,borderRadius:8,padding:"10px 14px",marginBottom:10}),T:{accent:"#ff6b35",text:"#fff9f0",sub:"#887766",border:"rgba(255,107,53,0.2)",bg:"#0d1f0d"}}
